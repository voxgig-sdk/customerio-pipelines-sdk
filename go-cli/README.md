# customerio-pipelines-cli

boru-driven command-line client **and** interactive REPL for the CustomerioPipelines
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/customerio-pipelines-cli)
make build

# 2. See usage (words, entities, env vars)
./customerio-pipelines-cli --help
...
CUSTOMERIO_PIPELINES_BASE=https://api.example.com ./customerio-pipelines-cli --help

# 6. No arguments -> interactive REPL
./customerio-pipelines-cli
customerio-pipelines> /help
customerio-pipelines> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/customerio-pipelines-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/customerio-pipelines-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `customerio-pipelines>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `alia`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/customerio-pipelines-cli`. |
...
(`./customerio-pipelines-cli <expr>`) and interactively (the REPL), and expressions compose the
