// Command CustomerioPipelines-cli is a CLI + REPL for the CustomerioPipelines SDK,
...
const prompt = "customerio-pipelines"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "alia batch group identify page screen track"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CUSTOMERIO_PIPELINES_APIKEY (api key), CUSTOMERIO_PIPELINES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CUSTOMERIO_PIPELINES_APIKEY carries the API key and
	// CUSTOMERIO_PIPELINES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CUSTOMERIO_PIPELINES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CUSTOMERIO_PIPELINES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCustomerioPipelinesSDK(opts)
