...
class CustomerioPipelinesMakeError
{
    public static function call(?CustomerioPipelinesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CustomerioPipelinesContext([], null);
        }
        $op = $ctx->op ?? new CustomerioPipelinesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CustomerioPipelinesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CustomerioPipelinesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CustomerioPipelinesError('', $msg, $ctx);
        ...
        if ($err instanceof CustomerioPipelinesError) {
