...
from projectname_sdk import CustomerioPipelinesSDK
from projectname_sdk.core.spec import CustomerioPipelinesSpec
from projectname_sdk.core.result import CustomerioPipelinesResult
from projectname_sdk.core.operation import CustomerioPipelinesOperation
from projectname_sdk.core.error import CustomerioPipelinesError
from projectname_sdk.feature.base_feature import CustomerioPipelinesBaseFeature
...
_runner = makeRunner(TEST_JSON_FILE, CustomerioPipelinesSDK.test(None, None))
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return CustomerioPipelinesError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_exists(self):
        client = CustomerioPipelinesSDK.test(None, None)
        utility = client._utility
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = CustomerioPipelinesResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = CustomerioPipelinesSDK.test(None, None)
        ...
        class TestInitFeature(CustomerioPipelinesBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(CustomerioPipelinesBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = CustomerioPipelinesSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = CustomerioPipelinesSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = CustomerioPipelinesSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = CustomerioPipelinesSDK.test(None, setup.get('a') or {})
    ...
    def test_operator_basic(self):
        def subject(vin):
            if not isinstance(vin, dict):
                vin = {}
            op = CustomerioPipelinesOperation(vin)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = CustomerioPipelinesSDK.test(None, setup.get('a') or {})
