...
const { CustomerioPipelinesSDK } = require('..')
...
describe('netsim', () => {
  ...
  test('offline simulation fails the request', async () => {
    const sdk = CustomerioPipelinesSDK.test({ net: { offline: true } })
  ...
  test('failStatus simulation surfaces the error status', async () => {
    const sdk = CustomerioPipelinesSDK.test({ net: { failTimes: 1, failStatus: 503 } })
    const res = await sdk.direct({ path: '/ping' })
    equal(res.ok, false)
  ...
  test('latency simulation delays the request', async () => {
    const delay = 60
    const sdk = CustomerioPipelinesSDK.test({ net: { latency: delay } })
  ...
  test('a plain test SDK still works with no net simulation', async () => {
    const sdk = CustomerioPipelinesSDK.test()
    equal(null !== sdk, true)
  })
})
