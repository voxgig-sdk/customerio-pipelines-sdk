# CustomerioPipelines Python SDK



The Python SDK for the CustomerioPipelines API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Alia()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/customerio-pipelines-sdk/releases)) or
...
client = CustomerioPipelinesSDK(options)
```
...
| --- | --- | --- |
| `previousId` | `str` | The anonymousId or userId value that you want to merge into the canonical profile. |
| `userId` | `str` | The userId that you want to keep. |

#### Example: Create

```python
alia = client.Alia().create({
    "previousId": "example_previousId",  # str
...
| Field | Type | Description |
| --- | --- | --- |
| `batch` | `list` | A group of requests you want to send to Data Pipelines in the call. |
...
```
py/
├── customeriopipelines_sdk.py         -- Main SDK module
...
The main module (`customeriopipelines_sdk`) exports the SDK class.
