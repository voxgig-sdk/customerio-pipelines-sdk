# CustomerioPipelines SDK test runner
...
class CustomerioPipelinesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CustomerioPipelinesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CustomerioPipelinesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CustomerioPipelinesTestRunner.getenv("CUSTOMERIO_PIPELINES_TEST_LIVE")
        override = CustomerioPipelinesTestRunner.getenv("CUSTOMERIO_PIPELINES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CustomerioPipelinesTestRunner.getenv(key)
        ...
        explain = CustomerioPipelinesTestRunner.getenv("CUSTOMERIO_PIPELINES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CUSTOMERIO_PIPELINES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CustomerioPipelinesTestRunner._test_control is not None:
            return CustomerioPipelinesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CustomerioPipelinesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CustomerioPipelinesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CustomerioPipelinesTestRunner.load_test_control()
...
def load_env_local():
    CustomerioPipelinesTestRunner.load_env_local()


def env_override(m):
    return CustomerioPipelinesTestRunner.env_override(m)


def entity_data(v):
    return CustomerioPipelinesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CustomerioPipelinesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CustomerioPipelinesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CustomerioPipelinesTestRunner.load_test_control()


def live_client_options():
    return CustomerioPipelinesTestRunner.live_client_options()


def live_delay_ms():
    return CustomerioPipelinesTestRunner.live_delay_ms()
