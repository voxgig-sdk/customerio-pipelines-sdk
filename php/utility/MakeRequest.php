...
class CustomerioPipelinesMakeRequest
{
    public static function call(CustomerioPipelinesContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new CustomerioPipelinesResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new CustomerioPipelinesResponse($fetched);
        } else {
