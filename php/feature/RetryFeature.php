...
// CustomerioPipelines SDK retry feature
...
class CustomerioPipelinesRetryFeature extends CustomerioPipelinesBaseFeature
{
    ...
    public function init(CustomerioPipelinesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CustomerioPipelinesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CustomerioPipelinesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CustomerioPipelinesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
