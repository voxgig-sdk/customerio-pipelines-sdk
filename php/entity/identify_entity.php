...
// CustomerioPipelines SDK Identify entity
...
class IdentifyEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new IdentifyEntity($this->_client, $opts);
    }

    /**
     * @param Identify|array $args Identify data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CustomerioPipelinesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Identify|array The current Identify data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Identify fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CustomerioPipelinesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Identify fields).
     */
    ...
    /**
     * Create a new Identify.
     *
     * @param IdentifyCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed IdentifyCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Identify|array The created Identify as an assoc-array at the
     *   SDK boundary; throws CustomerioPipelinesError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = CustomerioPipelinesHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
