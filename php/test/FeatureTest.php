...
// CustomerioPipelines SDK feature test
//
...
class FtCtrl extends CustomerioPipelinesControl
{
...
class FtClient
{
    ...
    private CustomerioPipelinesUtility $utility;

    public function __construct(CustomerioPipelinesUtility $utility, array $options)
    {
    ...
    public function get_utility(): CustomerioPipelinesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CustomerioPipelinesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CustomerioPipelinesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CustomerioPipelinesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CustomerioPipelinesConfig::make_feature($name);
    ...
    private static function buildUrl(CustomerioPipelinesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CustomerioPipelinesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CustomerioPipelinesResponse($fetched);
        }
    ...
    private function fail(CustomerioPipelinesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CustomerioPipelinesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
