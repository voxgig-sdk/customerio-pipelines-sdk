...
//    only - a typed CustomerioPipelinesContext there would make every ctx
...
final class CustomerioPipelinesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CustomerioPipelinesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CustomerioPipelinesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CustomerioPipelinesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CustomerioPipelinesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CustomerioPipelinesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CustomerioPipelinesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CustomerioPipelinesError(is_string($code) ? $code : '', $msg);
    }
