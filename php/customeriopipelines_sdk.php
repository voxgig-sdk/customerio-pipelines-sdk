...
class CustomerioPipelinesSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new CustomerioPipelinesUtility();
        $this->_utility = $utility;

        $config = CustomerioPipelinesConfig::shared_config();
        ...
        $feature_opts = CustomerioPipelinesHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CustomerioPipelinesHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CustomerioPipelinesFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CustomerioPipelinesFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CustomerioPipelinesUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CustomerioPipelinesHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CustomerioPipelinesHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CustomerioPipelinesHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
        ...
        $ctx->spec = new CustomerioPipelinesSpec([
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CustomerioPipelinesError($op . "_allow",
                "CustomerioPipelinesSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CustomerioPipelinesHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CustomerioPipelinesError("graphql_error",
                "CustomerioPipelinesSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Alia()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->alia()
    // resolves here too.
    public function Alia($data = null)
    {
        require_once __DIR__ . '/entity/alia_entity.php';
        if ($data === null) {
            if ($this->_alia === null) {
                $this->_alia = new AliaEntity($this, null);
            }
            return $this->_alia;
        }
        return new AliaEntity($this, $data);
    }
    ...
    private $_identify = null;

    // Canonical facade: $client->Identify()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->identify()
    // resolves here too.
    public function Identify($data = null)
    {
        require_once __DIR__ . '/entity/identify_entity.php';
        if ($data === null) {
            if ($this->_identify === null) {
                $this->_identify = new IdentifyEntity($this, null);
            }
            return $this->_identify;
        }
        return new IdentifyEntity($this, $data);
    }
    ...
    // Canonical facade: $client->Screen()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->screen()
    // resolves here too.
    public function Screen($data = null)
    {
        require_once __DIR__ . '/entity/screen_entity.php';
        if ($data === null) {
            if ($this->_screen === null) {
                $this->_screen = new ScreenEntity($this, null);
            }
            return $this->_screen;
        }
        return new ScreenEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new CustomerioPipelinesSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
