...
type Args struct {
	Entity string         `json:"entity" jsonschema:"alia | batch | group | identify | page | screen | track"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CustomerioPipelinesSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "customerio-pipelines_list",
		Description: "List records from CustomerioPipelines. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "customerio-pipelines_load",
		Description: "Load a single record from CustomerioPipelines. " +
...
func runOp(client *sdk.CustomerioPipelinesSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CustomerioPipelinesSDK, name string) (sdk.CustomerioPipelinesEntity, error) {
	switch strings.ToLower(name) {
	case "alia":
		return client.Alia(nil), nil
	case "batch":
		return client.Batch(nil), nil
	case "group":
		return client.Group(nil), nil
	case "identify":
		return client.Identify(nil), nil
	case "page":
		return client.Page(nil), nil
	case "screen":
		return client.Screen(nil), nil
	case "track":
		return client.Track(nil), nil
