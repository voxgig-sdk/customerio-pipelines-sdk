# customerio-pipelines-mcp

[MCP](https://modelcontextprotocol.io) server exposing the CustomerioPipelines SDK as
two agent tools — `customerio-pipelines_list` and `customerio-pipelines_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/customerio-pipelines-mcp)
make build

# 2. Provide credentials via the environment
export CUSTOMERIO_PIPELINES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user customerio-pipelines \
  -- /absolute/path/to/customerio-pipelines-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./customerio-pipelines-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// customerio-pipelines_list: first page of records
{ "entity": "alia" }
{ "entity": "alia", "query": { } }

// customerio-pipelines_load: one record by id
{ "entity": "alia", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/customerio-pipelines-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user customerio-pipelines \
     -- "$PWD"/dist/*/customerio-pipelines-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `customerio-pipelines_list` and `customerio-pipelines_load` tools now appear
   in new sessions. Ask the agent to *"list alia using customerio-pipelines"*
   and it calls `customerio-pipelines_list` with `{"entity":"alia"}`.
...
```sh
export CUSTOMERIO_PIPELINES_APIKEY=sk_live_xxx            # API key
export CUSTOMERIO_PIPELINES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `customerio-pipelines_list` tool
...
### Call the `customerio-pipelines_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `customerio-pipelines_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `customerio-pipelines_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CUSTOMERIO_PIPELINES_APIKEY` | API key sent with every request. |
| `CUSTOMERIO_PIPELINES_BASE` | Optional override of the API base URL. |

### Entities

The 7 entities valid as the `entity` argument:

alia | batch | group | identify | page | screen | track

### Smoke test via HTTP (raw JSON-RPC)

```sh
./customerio-pipelines-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"customerio-pipelines_load","arguments":{"entity":"alia","query":{"id":1}}}}'
```
