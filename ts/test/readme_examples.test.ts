...
const SDK_NAME = 'CustomerioPipelinesSDK'
...
const TEST_SEED = {"entity":{"alia":{"test01":{"id":"test01"}},"batch":{"test01":{"id":"test01"}},"group":{"test01":{"id":"test01"}},"identify":{"test01":{"id":"test01"}},"page":{"test01":{"id":"test01"}},"screen":{"test01":{"id":"test01"}},"track":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+CustomerioPipelinesSDK\b/.test(code) ||
    /\bCustomerioPipelinesSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CustomerioPipelinesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CustomerioPipelinesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
