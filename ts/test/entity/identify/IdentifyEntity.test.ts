...
describe('IdentifyEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    // CREATE
    const identify_ref01_ent = client.Identify()
    let identify_ref01_data = setup.data.new.identify['identify_ref01']

    identify_ref01_data = (await identify_ref01_ent.create(identify_ref01_data)).data()
    assert(null != identify_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/identify/IdentifyTestData.json')
  ...
  let idmap = transform(
    ['identify01','identify02','identify03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const env = envOverride({
    'CUSTOMERIO_PIPELINES_TEST_IDENTIFY_ENTID': idmap,
  ...
  idmap = env['CUSTOMERIO_PIPELINES_TEST_IDENTIFY_ENTID']

  const live = 'TRUE' === env.CUSTOMERIO_PIPELINES_TEST_LIVE

  const transport = createLiveTransport()
  if (live) {
    const rawIds = process.env['CUSTOMERIO_PIPELINES_TEST_IDENTIFY_ENTID']
