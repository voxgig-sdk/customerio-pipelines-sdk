...
func TestTrackEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_PIPELINES_TEST_TRACK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		trackRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "track"}), "track_ref01"))

		trackRef01DataResult, err := trackRef01Ent.Create(trackRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		trackRef01Data = core.ToMapAny(entityData(trackRef01DataResult))
		if trackRef01Data == nil {
...
func trackBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_PIPELINES_TEST_TRACK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_PIPELINES_TEST_TRACK_ENTID"])
