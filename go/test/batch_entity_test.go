...
func TestBatchEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_PIPELINES_TEST_BATCH_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		batchRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "batch"}), "batch_ref01"))
...
func batchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_PIPELINES_TEST_BATCH_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_PIPELINES_TEST_BATCH_ENTID"])
