...
func TestGroupEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_PIPELINES_TEST_GROUP_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func groupBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_PIPELINES_TEST_GROUP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_PIPELINES_TEST_GROUP_ENTID"])
