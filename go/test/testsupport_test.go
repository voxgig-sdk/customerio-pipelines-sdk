...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CUSTOMERIO_PIPELINES_TEST_LIVE") == "TRUE" ||
		os.Getenv("CUSTOMERIO_PIPELINES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CUSTOMERIO_PIPELINES_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client        *sdk.CustomerioPipelinesSDK
	data          map[string]any
