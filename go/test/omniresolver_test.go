...
import (
	...
	omni "github.com/voxgig-sdk/customerio-pipelines-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CustomerioPipelinesError); is {
				out := map[string]any{
					"name":    "CustomerioPipelinesError",
...
func omniCtx(arg any, client *sdk.CustomerioPipelinesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CustomerioPipelinesSDK); is {
			client = live
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.CustomerioPipelinesError{Msg: msg}
			}
		}
	}
...
func omniErrArg(arg any) error {
	...
	return &sdk.CustomerioPipelinesError{Msg: msg, Code: code}
}
