...
func NewBatchEntity(client *core.CustomerioPipelinesSDK, entopts map[string]any) *BatchEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *BatchEntity) GetName() string { return e.name }
...
func (e *BatchEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *BatchEntity) Deleted() bool {
	return e.deleted
}


func (e *BatchEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewBatchEntity(e.client, opts)
}

func (e *BatchEntity) Data(args ...any) any {
...
func (e *BatchEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Batch; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *BatchEntity) DataTyped(data ...Batch) Batch {
	if len(data) > 0 {
		return typedFrom[Batch](e.Data(asMap(data[0])))
	}
	return typedFrom[Batch](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Batch (all fields
// optional at the wire level).
func (e *BatchEntity) MatchTyped(match ...Batch) Batch {
	if len(match) > 0 {
		return typedFrom[Batch](e.Match(asMap(match[0])))
	}
	return typedFrom[Batch](e.Match())
}
...
func (e *BatchEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *BatchEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}


func (e *BatchEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}



func (e *BatchEntity) Create(reqdata map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// BatchCreateData and returns an Batch. It delegates to the untyped
// Create (identical runtime) and converts at the typed boundary.
func (e *BatchEntity) CreateTyped(reqdata BatchCreateData, ctrl map[string]any) (Batch, error) {
	res, err := e.Create(asMap(reqdata), ctrl)
	if err != nil {
		return Batch{}, err
	}
	return typedFrom[Batch](res), nil
}



func (e *BatchEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *BatchEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *BatchEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
