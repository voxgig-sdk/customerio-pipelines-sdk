...
function BatchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BatchEntity)
  self._name = "batch"
  self._client = client
...
function BatchEntity:get_name()
  return self._name
end


function BatchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BatchEntity.new(self._client, opts)
end
...
function BatchEntity:mark_deleted()
  self._deleted = true
end


function BatchEntity:deleted()
  return true == self._deleted
end


function BatchEntity:data_set(args)
  if args ~= nil then
...
function BatchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BatchEntity:match_set(args)
  if args ~= nil then
...
function BatchEntity:match_get()
...
function BatchEntity:stream(action, args, callopts)
...
---@param reqdata BatchCreateData
---@param ctrl? table
---@return Batch
---@return string? err
function BatchEntity:create(reqdata, ctrl)
  local utility = self._utility
