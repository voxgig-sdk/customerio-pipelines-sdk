...
local TrackEntity = {}
TrackEntity.__index = TrackEntity


function TrackEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TrackEntity)
  self._name = "track"
  self._client = client
...
function TrackEntity:get_name()
  return self._name
end


function TrackEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TrackEntity.new(self._client, opts)
end
...
function TrackEntity:mark_deleted()
  self._deleted = true
end


function TrackEntity:deleted()
  return true == self._deleted
end


function TrackEntity:data_set(args)
  if args ~= nil then
...
function TrackEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TrackEntity:match_set(args)
  if args ~= nil then
...
function TrackEntity:match_get()
...
function TrackEntity:stream(action, args, callopts)
...
---@param reqdata TrackCreateData
---@param ctrl? table
---@return Track
---@return string? err
function TrackEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function TrackEntity:_run_op(ctx, post_done)
  local utility = self._utility
