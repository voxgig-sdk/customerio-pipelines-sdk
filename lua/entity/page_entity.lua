...
local PageEntity = {}
PageEntity.__index = PageEntity


function PageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PageEntity)
  self._name = "page"
  self._client = client
...
function PageEntity:get_name()
  return self._name
end


function PageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PageEntity.new(self._client, opts)
end
...
function PageEntity:mark_deleted()
  self._deleted = true
end


function PageEntity:deleted()
  return true == self._deleted
end


function PageEntity:data_set(args)
  if args ~= nil then
...
function PageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PageEntity:match_set(args)
  if args ~= nil then
...
function PageEntity:stream(action, args, callopts)
...
---@param reqdata PageCreateData
---@param ctrl? table
---@return Page
---@return string? err
function PageEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PageEntity:_run_op(ctx, post_done)
  local utility = self._utility
