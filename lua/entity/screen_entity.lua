...
local ScreenEntity = {}
ScreenEntity.__index = ScreenEntity


function ScreenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ScreenEntity)
  self._name = "screen"
  self._client = client
...
function ScreenEntity:get_name()
  return self._name
end


function ScreenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ScreenEntity.new(self._client, opts)
end
...
function ScreenEntity:mark_deleted()
  self._deleted = true
end


function ScreenEntity:deleted()
  return true == self._deleted
end


function ScreenEntity:data_set(args)
  if args ~= nil then
...
function ScreenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ScreenEntity:match_set(args)
  if args ~= nil then
...
function ScreenEntity:match_get()
...
function ScreenEntity:stream(action, args, callopts)
...
---@param reqdata ScreenCreateData
---@param ctrl? table
---@return Screen
---@return string? err
function ScreenEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ScreenEntity:_run_op(ctx, post_done)
  local utility = self._utility
