...
local AliaEntity = {}
AliaEntity.__index = AliaEntity


function AliaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AliaEntity)
  self._name = "alia"
  self._client = client
...
function AliaEntity:get_name()
  return self._name
end


function AliaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AliaEntity.new(self._client, opts)
end
...
function AliaEntity:mark_deleted()
  self._deleted = true
end


function AliaEntity:deleted()
  return true == self._deleted
end


function AliaEntity:data_set(args)
  if args ~= nil then
...
function AliaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AliaEntity:match_set(args)
  if args ~= nil then
...
function AliaEntity:stream(action, args, callopts)
...
---@param reqdata AliaCreateData
---@param ctrl? table
---@return Alia
---@return string? err
function AliaEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function AliaEntity:_run_op(ctx, post_done)
  local utility = self._utility
