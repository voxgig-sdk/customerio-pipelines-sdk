...
local GroupEntity = {}
GroupEntity.__index = GroupEntity


function GroupEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GroupEntity)
  self._name = "group"
  self._client = client
...
function GroupEntity:get_name()
  return self._name
end


function GroupEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GroupEntity.new(self._client, opts)
end
...
function GroupEntity:mark_deleted()
  self._deleted = true
end


function GroupEntity:deleted()
  return true == self._deleted
end


function GroupEntity:data_set(args)
  if args ~= nil then
...
function GroupEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GroupEntity:match_set(args)
  if args ~= nil then
...
function GroupEntity:match_get()
...
function GroupEntity:stream(action, args, callopts)
...
---@param reqdata GroupCreateData
---@param ctrl? table
---@return Group
---@return string? err
function GroupEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function GroupEntity:_run_op(ctx, post_done)
  local utility = self._utility
