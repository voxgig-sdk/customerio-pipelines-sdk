...
describe("GroupEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_PIPELINES_TEST_GROUP_ENTID JSON to run live")
      return
    end
    ...
    local group_ref01_data = helpers.to_map(vs.getprop(
...
function group_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CUSTOMERIO_PIPELINES_TEST_GROUP_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["CUSTOMERIO_PIPELINES_TEST_GROUP_ENTID"])
