...
require("customerio-pipelines_types")
...
local CustomerioPipelinesSDK = {}
CustomerioPipelinesSDK.__index = CustomerioPipelinesSDK
...
CustomerioPipelinesSDK._make_feature = _make_feature


function CustomerioPipelinesSDK.new(options)
  local self = setmetatable({}, CustomerioPipelinesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CustomerioPipelinesSDK:options_map()
...
function CustomerioPipelinesSDK:get_utility()
  return Utility.copy(self._utility)
end


function CustomerioPipelinesSDK:get_root_ctx()
  return self._rootctx
end


function CustomerioPipelinesSDK:prepare(fetchargs)
...
function CustomerioPipelinesSDK:direct(fetchargs)
...
function CustomerioPipelinesSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function CustomerioPipelinesSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "CustomerioPipelinesSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function CustomerioPipelinesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CustomerioPipelinesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CustomerioPipelinesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:Alia():list() / client:Alia():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CustomerioPipelinesSDK:Alia(data)
  local EntityMod = require("entity.alia_entity")
  if data == nil then
    if self._alia == nil then
      self._alia = EntityMod.new(self, nil)
    end
    return self._alia
  end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CustomerioPipelinesSDK:Batch(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CustomerioPipelinesSDK:Group(data)
...
-- Idiomatic facade: client:Identify():list() / client:Identify():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CustomerioPipelinesSDK:Identify(data)
  local EntityMod = require("entity.identify_entity")
  if data == nil then
    if self._identify == nil then
      self._identify = EntityMod.new(self, nil)
    end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CustomerioPipelinesSDK:Page(data)
...
-- Idiomatic facade: client:Screen():list() / client:Screen():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CustomerioPipelinesSDK:Screen(data)
  local EntityMod = require("entity.screen_entity")
  if data == nil then
    if self._screen == nil then
      self._screen = EntityMod.new(self, nil)
    end
    return self._screen
  end
  return EntityMod.new(self, data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CustomerioPipelinesSDK:Track(data)
...
function CustomerioPipelinesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CustomerioPipelinesSDK.new(sdkopts)
  sdk.mode = "test"
