# CustomerioPipelines Lua SDK



The Lua SDK for the CustomerioPipelines API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Alia()` — each with the same small set of operations (`create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/customerio-pipelines-sdk/releases)),
...
```lua
-- Create
local created, err = client:Alia():create({ previousId = "example_previousId", userId = "example_userId" })
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local alia, err = client:Alia():list()
    if err then error(err) end
    -- alia is the record list
...
Create an instance: `local alia = client:Alia(nil)`
...
```lua
local alia, err = client:Alia():create({
  previousId = "example_previousId", -- string
...
| Field | Type | Description |
| --- | --- | --- |
| `batch` | `table` | A group of requests you want to send to Data Pipelines in the call. |
...
Create an instance: `local identify = client:Identify(nil)`
...
```lua
local identify, err = client:Identify():create({
})
```
...
```lua
local page, err = client:Page():create({
})
```
...
Create an instance: `local screen = client:Screen(nil)`
...
```lua
local screen, err = client:Screen():create({
})
```
...
```lua
local track, err = client:Track():create({
})
```
...
```
lua/
├── customerio-pipelines_sdk.lua    -- Main SDK module
...
The main module (`customerio-pipelines_sdk`) exports the SDK constructor
